...
const pythonCompensation: CodeLine[] = [
  ...
  { text: '    for comp in reversed(compensations):', indent: 3, step: 'release_payment_hold' },
  { text: '        await comp()', indent: 3, step: 'release_payment_hold' },
]

const pythonHighlighting = [
  { pattern: /\b(class|async|def|await|if|return|except|lambda|self|for|in|try)\b/g, className: 'text-purple-400 font-semibold' },
  { pattern: /\b(workflow|RetryPolicy)\b/g, className: 'text-blue-300' },
  { pattern: /(validate_order|validate_store|authorize_payment|clear_cart|submit_to_store|capture_payment|release_payment_hold|release_hold|wait_condition|execute_activity|order_ready|compensations|reversed|append)\b/g, className: 'text-amber-300' },
...
const goCode: CodeLine[] = [
  ...
  { text: '    order OrderInput) (Result, error) {', indent: 0 },
  { text: '', indent: 0, isBlank: true },
  { text: 'var compensations []func(workflow.Context) error', indent: 1 },
  { text: '', indent: 0, isBlank: true },
  { text: '// Validate order and store', indent: 1, isComment: true },
  { text: 'err := workflow.ExecuteActivity(ctx,', indent: 1, step: 'validate_order' },
  { text: '    "ValidateOrder", order).Get(ctx, nil)', indent: 1, step: 'validate_order' },
  { text: 'err = workflow.ExecuteActivity(ctx,', indent: 1, step: 'validate_store' },
  { text: '    "ValidateStore", order).Get(ctx, nil)', indent: 1, step: 'validate_store' },
  { text: '', indent: 0, isBlank: true },
  { text: '// Hold payment — register compensation first', indent: 1, isComment: true },
  { text: 'compensations = append(compensations, releaseHold)', indent: 1, step: 'authorize_payment' },
  { text: 'err = workflow.ExecuteActivity(ctx,', indent: 1, step: 'authorize_payment' },
  { text: '    "AuthorizePayment", order).Get(ctx, &auth)', indent: 1, step: 'authorize_payment' },
  { text: '', indent: 0, isBlank: true },
  { text: 'workflow.ExecuteActivity(ctx,', indent: 1, step: 'clear_cart' },
  { text: '    "ClearCart", order).Get(ctx, nil)', indent: 1, step: 'clear_cart' },
  { text: '', indent: 0, isBlank: true },
  { text: '// Submit to store — retries automatically', indent: 1, isComment: true },
  { text: 'submitCtx := workflow.WithActivityOptions(ctx,', indent: 1, step: 'submit_to_store' },
  { text: '    RetryPolicy{MaxAttempts: 10})', indent: 1, step: 'submit_to_store' },
  { text: 'workflow.ExecuteActivity(submitCtx,', indent: 1, step: 'submit_to_store' },
  { text: '    "SubmitToStore", order).Get(ctx, nil)', indent: 1, step: 'submit_to_store' },
  { text: '', indent: 0, isBlank: true },
  { text: '// Wait for signal — human in the loop', indent: 1, isComment: true },
  { text: 'workflow.GetSignalChannel(ctx, "order_ready")', indent: 1, step: 'order_ready' },
  ...
  { text: '    "CapturePayment", order).Get(ctx, nil)', indent: 1, step: 'capture_payment' },
  { text: '', indent: 0, isBlank: true },
...
const goCompensation: CodeLine[] = [
  { text: '', indent: 0, isBlank: true },
  { text: '// Saga: run compensations in reverse', indent: 0, isComment: true },
  { text: 'disconnected, _ := workflow.NewDisconnectedContext(ctx)', indent: 0, step: 'release_payment_hold' },
  { text: 'for i := len(compensations) - 1; i >= 0; i-- {', indent: 0, step: 'release_payment_hold' },
  { text: '    compensations[i](disconnected)', indent: 1, step: 'release_payment_hold' },
  { text: '}', indent: 0, step: 'release_payment_hold' },
]

const goHighlighting = [
  { pattern: /\b(func|var|err|return|for|if|range|nil|true|false|append|len)\b/g, className: 'text-purple-400 font-semibold' },
  { pattern: /\b(workflow|temporal|RetryPolicy|Context|OrderInput|Result)\b/g, className: 'text-blue-300' },
  { pattern: /(ExecuteActivity|WithActivityOptions|GetSignalChannel|Receive|Get|NewDisconnectedContext|MaxAttempts)\b/g, className: 'text-amber-300' },
  { pattern: /("ValidateOrder"|"ValidateStore"|"AuthorizePayment"|"ClearCart"|"SubmitToStore"|"CapturePayment"|"order_ready")/g, className: 'text-green-300' },
  { pattern: /\b(\d+)\b/g, className: 'text-cyan-300' },
]
...
const javaHighlighting = [
  { pattern: /\b(public|interface|void)\b/g, className: 'text-purple-400 font-semibold' },
  { pattern: /@\w+/g, className: 'text-yellow-400' },
  { pattern: /\b(OrderWorkflow|Result|OrderInput)\b/g, className: 'text-blue-300' },
]
