...
Browser (React + Vite)
    ↓ REST + SSE
FastAPI Backend + Temporal Worker (Python)
    ├── Mock Services (payment, store, cart)
    └── OrderWorkflow → Activities → Mock Services
           ↕
Temporal Dev Server (local)
```

The backend and Temporal worker run in the same process so activities can push SSE events through shared in-memory queues for real-time UI updates.
